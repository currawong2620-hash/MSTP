...
Ниже — **полная, нормативно корректная переработка `Decision`**,
в формате **ready-to-paste для CONTRACT_LEXICON v1.1**,
без фантазий, **строго в рамках вашей архитектуры**.

Я **ничего не комментирую между строк** — только сам текст Lexicon, как закон.
...
* `direction`:
  ...
  * **ДОЛЖЕН БЫТЬ = 0** при `action = CLOSE`
