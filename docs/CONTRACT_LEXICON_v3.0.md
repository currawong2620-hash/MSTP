# 🟥 CONTRACT_LEXICON

## Architecture Data Dictionary **v3.0**

**Status:** NORMATIVE
**Scope:** All system contracts and modules
**Change policy:** versioned only
**Authority:** SSP v3.0

---

# -1. ARCHITECTURE TYPE GOVERNANCE

**Status:** NORMATIVE
**Scope:** All architecture data contracts
**Authority:** SSP v3.0

---

## -1.1 Single Source of Truth

Настоящий документ является **ЕДИНСТВЕННЫМ источником истины**
о допустимых архитектурных типах данных, их полях и enum-значениях.

Любой тип, поле или enum-элемент,
отсутствующий в данном документе,
считается **ЗАПРЕЩЁННЫМ**.

---

## -1.2 Ownership of Architecture Types

Архитектурные типы данных:

* **НЕ определяются** в исполняемом коде
* **НЕ определяются** в технических заданиях
* **НЕ принадлежат** отдельным ролям или модулям

Архитектурные типы принадлежат
**архитектуре системы в целом**.

---

## -1.3 Master-Chat Authority Rule

Ни один Master-Chat **НЕ ИМЕЕТ ПРАВА**:

* вводить новый архитектурный тип
* добавлять поле существующего типа
* вводить новый enum или enum-значение
* переименовывать существующие элементы

если соответствующее изменение
**НЕ зафиксировано новой версией Lexicon**.

При отсутствии подходящего типа
**РАБОТА ДОЛЖНА БЫТЬ ОСТАНОВЛЕНА**.

---

## -1.4 Separation from Implementation

Код **МОЖЕТ ТОЛЬКО ИСПОЛЬЗОВАТЬ**
архитектурные типы, определённые в Lexicon.

Код **НЕ ИМЕЕТ ПРАВА**:

* определять структуру типов
* расширять поля
* интерпретировать типы «по смыслу»

---

## -1.5 Audit Enforcement Rule

Любой Audit-Chat обязан:

* отклонить код
* отклонить ТЗ
* отклонить обсуждение

если используется тип, поле или enum,
отсутствующие в Lexicon.

---

## -1.6 Interpretative Prohibition

При любом сомнении,
соответствует ли существующий тип требуемой сущности,
**интерпретация запрещена**.

В этом случае:

1. код не пишется
2. ТЗ не формируется
3. сначала обновляется Lexicon

---

# 0. PURPOSE

Документ фиксирует **ЕДИНСТВЕННЫЕ допустимые имена**
архитектурных типов данных, их полей и enum-значений.

Любое имя, отсутствующее здесь,
считается **ЗАПРЕЩЁННЫМ**.

---

# 1. GENERAL NAMING RULES

* Имена типов и полей **буквально обязательны**
* Регистр обязателен
* Синонимы запрещены
* Контекст задаётся структурой, а не именем поля

---

# 2. ARCHITECTURAL ROOT TYPES

В системе допускаются **ТОЛЬКО** следующие архитектурные типы:

* `Snapshot`
* `Intent`
* `PolicyAdjustedIntent`
* `Decision`
* `ExecutionResult`
* `Feedback`

---

# 3. SNAPSHOT

## 3.1 Purpose

`Snapshot` фиксирует наблюдаемое состояние мира
в **один момент времени**.

---

## 3.2 Structure

```
Snapshot
├── market
├── position
├── time
├── constraints
└── trends
```

---

## 3.3 Snapshot.market (MarketSnapshot)

| Field       | Type      | Description             |
| ----------- | --------- | ----------------------- |
| symbol      | symbol    | торговый инструмент     |
| tf          | timeframe | таймфрейм               |
| open        | price     | цена открытия бара      |
| high        | price     | максимум бара           |
| low         | price     | минимум бара            |
| close       | price     | цена закрытия бара      |
| volume      | volume    | объём бара              |
| bid         | price     | текущий bid             |
| ask         | price     | текущий ask             |
| spread      | price     | ask − bid               |
| last_closes | price[3]  | последние закрытые бары |
| point_size  | price     | минимальный ценовой шаг |

### last_closes — Normative Semantics

* содержит **ТОЛЬКО закрытые бары**
* длина массива **фиксирована = 3**
* порядок элементов строго обязателен
* значения формируются **Observer**
* downstream роли **НЕ МОГУТ** модифицировать

---

## 3.4 Snapshot.position (PositionSnapshot)

| Field               | Type      | Description                            |
| ------------------- | --------- | -------------------------------------- |
| has_position        | bool      | наличие позиции                        |
| direction           | direction | +1 / −1                                |
| volume              | volume    | объём позиции                          |
| entry_price         | price     | цена входа                             |
| floating_pnl        | money     | плавающий результат                    |
| trailing_stop_price | price     | последний подтверждённый trailing stop |

---

## 3.5 Snapshot.time (TimeContext)

| Field                  | Type      | Description                 |
| ---------------------- | --------- | --------------------------- |
| timestamp              | timestamp | текущее время               |
| is_new_bar             | bool      | флаг нового бара            |
| bars_since_entry       | count     | баров с момента входа       |
| bars_since_last_action | count     | баров с последнего действия |

---

## 3.6 Snapshot.constraints (MarketConstraints)

| Field              | Type   | Description        |
| ------------------ | ------ | ------------------ |
| min_lot            | volume | минимальный лот    |
| lot_step           | volume | шаг лота           |
| min_stop           | price  | минимальный стоп   |
| is_trading_allowed | bool   | торговля разрешена |

---

## 3.7 Snapshot.trends

**(SSP v3.0 approved extension)**

```
trends
├── count
└── items[]
```

| Field | Type        | Description                |
| ----- | ----------- | -------------------------- |
| count | count       | количество таймфреймов     |
| items | TrendInfo[] | упорядоченные тренд-записи |

### Normative Rules

* `count >= 1`
* `items.length == count`
* порядок `items[]` = порядок input-строки
* immutable
* формируется **ТОЛЬКО Observer**

---

# 4. INTENT

## Structure

```
Intent
├── type
├── direction
└── confidence
```

| Field      | Type        | Description         |
| ---------- | ----------- | ------------------- |
| type       | intent_type | категория намерения |
| direction  | direction   | −1 / 0 / +1         |
| confidence | ratio       | 0.0 … 1.0           |

### intent_type (enum)

* `NO_ACTION`
* `WANT_OPEN`
* `WANT_CLOSE`
* `WANT_HOLD`

---

# 5. POLICYADJUSTEDINTENT

## Structure

```
PolicyAdjustedIntent
├── type
├── direction
├── volume
└── tag
```

| Field     | Type        | Description            |
| --------- | ----------- | ---------------------- |
| type      | intent_type | категория              |
| direction | direction   | направление            |
| volume    | volume      | допустимый объём       |
| tag       | string      | идентификатор политики |

### Normative Rules

* не является командой исполнения
* `WANT_OPEN` → `direction ∈ {+1, −1}`, `volume` обязателен
* `WANT_CLOSE` → `direction = 0`, `volume` обязателен

---

# 6. DECISION

## Structure

```
Decision
├── status
├── action
├── direction
├── volume
├── symbol
└── reason
```

| Field     | Type            | Description  |
| --------- | --------------- | ------------ |
| status    | decision_status | итог         |
| action    | decision_action | OPEN / CLOSE |
| direction | direction       | направление  |
| volume    | volume          | объём        |
| symbol    | symbol          | инструмент   |
| reason    | string          | код причины  |

### decision_status

* `ACCEPT`
* `REJECT`
* `MODIFY`

### decision_action

* `OPEN`
* `CLOSE`

---

# 7. EXECUTIONRESULT

```
ExecutionResult
├── status
├── filled_volume
└── price
```

| Field         | Type             |
| ------------- | ---------------- |
| status        | execution_status |
| filled_volume | volume           |
| price         | price            |

### execution_status

* `EXECUTED`
* `REJECTED`
* `PARTIAL`
* `FAILED`

---

# 8. FEEDBACK

```
Feedback
├── event
├── pnl
└── message
```

| Field   | Type           |
| ------- | -------------- |
| event   | feedback_event |
| pnl     | money          |
| message | string         |

### feedback_event

* `POSITION_OPENED`
* `POSITION_CLOSED`
* `STOP_HIT`
* `TARGET_HIT`
* `ACTION_REJECTED`

---

# 9. TrendInfo

```
TrendInfo
├── timeframe
├── regime
├── direction
└── confidence
```

| Field      | Type         | Description |
| ---------- | ------------ | ----------- |
| timeframe  | timeframe    | таймфрейм   |
| regime     | trend_regime | режим рынка |
| direction  | direction    | −1 / 0 / +1 |
| confidence | ratio        | 0.0 … 1.0   |

---

## 9.1 trend_regime (enum)

```
trend_regime
├── TREND
├── RANGE
└── UNKNOWN
```

---

# 10. SEMANTIC TYPES (REFERENCE)

| Type      | Meaning               |
| --------- | --------------------- |
| price     | ценовое значение      |
| volume    | объём                 |
| money     | денежный результат    |
| ratio     | 0.0 … 1.0             |
| count     | целочисленный счётчик |
| timestamp | время                 |
| direction | −1 / 0 / +1           |
| timeframe | таймфрейм             |
| symbol    | торговый инструмент   |

---

# 11. GLOBAL PROHIBITIONS

❌ добавление полей без новой версии Lexicon
❌ переименование элементов
❌ синонимы
❌ версии в именах типов

---

**END OF CONTRACT_LEXICON v3.0**
